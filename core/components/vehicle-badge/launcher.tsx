...
export function VehicleBadgeLauncher({ variant = 'default', locale }: VehicleBadgeLauncherProps) {
    ...
    <div className="relative">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          {/* Backdrop */}
          <div
            className={`absolute inset-0 bg-black transition-opacity duration-200 ${animateIn ? 'opacity-50' : 'opacity-0'}`}
          ...
          {/* Modal */}
          <div
            className={`relative w-full max-w-2xl transform transition-all duration-200 ease-out ${animateIn ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
              }`}
          >
            <div className="rounded-lg bg-[#FFCC00] p-6 shadow-2xl">
              {/* Header */}
              <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#2F3740]">
                    <svg aria-hidden viewBox="0 0 24 24" className="h-8 w-8 text-white">
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M18.5243 12.0005C18.5243 15.8665 15.4687 19.0005 11.6993 19.0005C11.6993 15.6865 12.6743 11.5005 17.5493 11.0005H18.4551C18.5013 11.3317 18.5244 11.6659 18.5243 12.0005V12.0005Z"
                  ...
                  <div className="flex flex-col leading-tight">
                    <h2 className="text-xl font-bold text-black md:text-2xl">Mon garage</h2>
                    <p className="text-sm text-black md:text-base">Sélectionne ton véhicule</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="rounded-full p-2 text-[#2F3740] transition hover:bg-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-900"
                  aria-label="Fermer"
                >
